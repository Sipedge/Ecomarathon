...
class CreateUser(ft.View):
    def __init__(self, config: Config):
        super().__init__(route="/create_user", padding=0) 
        self.config = config
        self.expand = True
        self.bgcolor="#F5EAD4"
    ...
    async def create(self, e):
        ...
        if self.config._errors == "Домен/IP указан неверно":
            alert = AlertDialog_spawn(self.config._errors, 'Выход', my_page=self.page, on_click=exit_app)
        ...
        if self.config._errors == "Такое имя уже занято. Введите другое имя.":
            ...
            alert.alert_spawn(self.config)
            return 0
        await create_server(self.config)
